...
{\heiti 不可见任务。}不可见任务是指过程模型中存在，但是实际执行日志中并不存在的任务。RORU将不可见任务也作为普通可见任务处理，然而过程模型中可能存在充当路由功能的无意义任务。另外，在变迁执行序列中信息的缺失和噪声的存在都可能导致挖掘过程产生不可见任务。目前，RORU并没有考虑不可见任务对过程模型行为的影响，所以无法正确处理含有不可见任务的模型。
